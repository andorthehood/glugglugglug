...
export const FULLSCREEN_VERTEX_SHADER_SOURCE = `#version 300 es
	...
	out vec2 v_topLeftScreenCoord;

	void main() {
		gl_Position = vec4(a_position, 0.0, 1.0);
		v_screenCoord = (a_position + 1.0) * 0.5;
		v_textureCoord = v_screenCoord;
		v_topLeftScreenCoord = vec2(v_screenCoord.x, 1.0 - v_screenCoord.y);
	}
`;

/** GPU geometry owned by the shader-underlay plugin. */
...
/**
 * Creates reusable fullscreen geometry for the shader-underlay plugin.
 *
...
/**
 * Deletes the shader-underlay plugin's reusable fullscreen geometry.
 *
 * @param gl - WebGL2 context that owns the geometry.
...
/**
 * Establishes the default-framebuffer state required by the underlay pass.
 *
